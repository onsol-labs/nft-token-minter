...
export const ON_CHAIN_METADATA = {
    ...
    creators: [
        {
            address: config.MINT_AUTHORITY.publicKey,
            verified: true,
            share: 100,
        },
    ],
...
export async function mintNewNFT(
    destinationWallet: PublicKey = config.MINT_AUTHORITY.publicKey,
) {
    ...
    console.log(
        `transaction: https://solscan.io/tx/${txn_id}${
            isDevnet ? "?cluster=devnet" : ""
        } `,
    );
    console.log(
        `token link: https://solscan.io/token/${mintKeypair.publicKey.toString()}${
            isDevnet ? "?cluster=devnet" : ""
        }`,
    );
...
export async function mintTokensToOnePubkey(
    mintPublicKey: PublicKey,
    ...
    console.log(
        `minted ${config.MINT_CONFIG.numberTokens} ${
            ON_CHAIN_METADATA.symbol
        } to ${userPublicKey.toString()}.`,
    );
    console.log(
        `https://solscan.io/tx/${txn_id}${isDevnet ? "?cluster=devnet" : ""}`,
    );
}
...
export async function tokenMultiMinter(
    mintPublicKey: PublicKey,
    holderWalletArray: walletAmountType[],
) {
    let counter = 0;
    ...
    for (const holderWallet of holderWalletArray) {
        ...
        if (counter === 5) {
            ...
            const txn_id = await config.CONNECTION.sendTransaction(
                mintToTransaction,
                [config.MINT_AUTHORITY],
            );
            console.log(
...
export async function nftMultiMinter(holderWalletArray: walletAmountType[]) {
    ...
    for (const holderWallet of holderWalletArray) {
        ...
        for (let i = 0; i < holderWallet.amount; i++) {
            ...
            const newMintTransaction: Transaction =
                await createNewMintTransaction(
                    config.CONNECTION,
                    config.MINT_AUTHORITY,
                    mintKeypair,
                    userPublicKey,
            ...
            const txn_id = await config.CONNECTION.sendTransaction(
                newMintTransaction,
                [config.MINT_AUTHORITY, mintKeypair],
            );
            console.log(
                `transaction: https://solscan.io/tx/${txn_id}${
                    isDevnet ? "?cluster=devnet" : ""
